...
from deepep_dispatch import (build_rank_buffers_ptr, deepep_owner_dispatch,
                             get_expert_owner)
...
def build_dispatch_projection(
    ...
    owner_table = torch.tensor(
        [
            get_expert_owner(e, num_experts, world_size)
            for e in range(num_experts)
        ],
        device=topk_idx.device,
        dtype=torch.int64,
    )
    owners_2d = owner_table[topk_idx]
    token_ids = torch.arange(
        num_tokens, device=topk_idx.device, dtype=torch.int64
    )
...
def build_reference(
    ...
    num_tokens_per_rank = all_hidden.shape[1]
    hidden_size = all_hidden.shape[2]
    num_tokens, topk = topk_idx.shape
    ref = torch.zeros(
...
def check_deepep_owner_dispatch():
    ...
    rank_buffers = build_rank_buffers_ptr(
        hidden_shard, NUM_EXPERTS, group=group
    )
    ...
    latencies = [
        begin_events[i].elapsed_time(end_events[i]) for i in range(WARMUP, LOOP)
    ]
    print(f"[DeePEP dispatch time in rank {rank}] {latencies} ms")

    gathered_hidden = [
        torch.empty_like(hidden_shard) for _ in range(world_size)
    ]
    dist.all_gather(gathered_hidden, hidden_shard, group=group)
    all_hidden = torch.stack(gathered_hidden, dim=0)
    ref = build_reference(
        all_hidden, topk_idx, scatter_idx, NUM_EXPERTS, rank, world_size
    )
    ...
    has_lp = hasattr(torch.ops._moe_C, "local_permute_copy")
    ...
    if has_lp:
        ...
        dist.barrier()
        ref_latencies = [
            ref_begin[i].elapsed_time(ref_end[i]) for i in range(LOOP)
        ]
        ...
        assert torch.equal(
            ...
            "Baseline local_permute vs deepep_owner_dispatch"
            f" mismatch at owned positions in rank {rank}"
        )
    else:
        avg_ref = None
    ...
    if rank == 0:
        ...
        if ENABLE_PROJECTION:
            ...
            print(
                f"\n[Projection] PCIe BW={CROSS_GPU_BW_GBPS:.1f} GB/s, "
                f"HBM BW={HBM_BW_GBPS:.1f} GB/s"
            )
            ...
            worst = max(
                range(world_size),
                key=lambda r: remote_fetch_ms[r] + topk_write_ms[r],
            )
            print(
