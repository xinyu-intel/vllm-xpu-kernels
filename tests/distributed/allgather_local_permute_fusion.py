...
Implements the algorithm: - Each rank knows the global topk_idx mapping - Each
rank writes its local hidden_shard to remap_hidden_states according to
scatter_idx - Allgather is performed directly into the correct positions in
remap_hidden_states - No redundant permute after allgather

The output remap_hidden_states is organized in expert-centric layout: all tokens
assigned to expert 0 come first, then expert 1, etc.

Usage:
    from allgather_local_permute_fusion import (
        compute_scatter_idx, allgather_local_permute_fusion,
...
try:
    ...
    _HAS_LOCAL_PERMUTE_KERNEL = \
        hasattr(torch.ops._moe_C, "local_permute_copy")
    _HAS_ALLGATHER_PERMUTE_KERNEL = \
        hasattr(torch.ops._moe_C, "allgather_permute")
    _HAS_ALLGATHER_WITH_SYMM_MEM_KERNEL = \
        hasattr(
        torch.ops._moe_C, "allgather_with_symm_mem"
    )
except ImportError:
...
def compute_scatter_idx(topk_idx: torch.Tensor, num_experts: int = None):
    ...
    The output layout groups tokens by expert: all tokens routed to expert 0 are
    placed first, then expert 1, etc. Within each expert's block, tokens appear
    in the order determined by stable sort of (expert_id, original_position).

    Args:
        topk_idx: [num_tokens, topk] int64 - expert assignment for each (token,
        k) num_experts: total number of experts (optional, inferred from
        topk_idx if None)

    Returns:
        scatter_idx: [num_tokens, topk] int32 - output position for each (token,
        k) expert_offsets: [num_experts + 1] int64 - start offset of each expert
        in output
    """
    ...
    expert_counts = torch.bincount(topk_flat.to(torch.int64),\
        minlength=num_experts)
    expert_offsets = torch.zeros(
        num_experts + 1, dtype=torch.int64, device=topk_idx.device
    )
...
def allgather_local_permute_fusion_native(
    ...
    (ring-ordered) and writes all (token, k) positions.

    Falls back to multi-kernel path (per-rank copy + local_permute_copy) when
    the fused kernel is not built.
    """
    if group is None:
    ...
    if _HAS_ALLGATHER_PERMUTE_KERNEL:
        assert rank_buffers_ptr is not None, (
            "rank_buffers_ptr is required; "
            "use build_allgather_rank_buffers_ptr()"
        )
        torch.ops._moe_C.allgather_permute(
            rank_buffers_ptr,
            scatter_idx,
            remap_hidden_states,
            rank,
        ...
        torch.ops._moe_C.local_permute_copy(
            hidden_shard,
            scatter_idx,
            local_token_offset,
            remap_hidden_states,
...
def allgather_local_permute_fusion_python(
    ...
    This keeps the original Python implementation for correctness checks and A/B
    performance comparison with the native fused kernel API.
    """
    if group is None:
        group = dist.group.WORLD
    if group_name is None:
...
def build_allgather_rank_buffers_ptr(
    ...
    """Precompute rank_buffers_ptr tensor for repeated allgather_permute
    calls."""
    if group is None:
        group = dist.group.WORLD
    if group_name is None:
...
def build_allgather_with_symm_mem_rank_buffers_ptr(
    ...
    workspace_size_bytes = (
        numel_per_rank * world_size * input_flat.element_size()
    )
    workspace = symm_mem.get_symm_mem_workspace(
        group_name, min_size=workspace_size_bytes
    )
...
def allgather_local_permute_fusion(
    ...
    Args:
        hidden_shard: [num_tokens_per_rank, hidden_size] local input topk_idx:
        [num_tokens, topk] global expert assignments scatter_idx: [num_tokens,
        topk] int32 - pre-computed output positions
                     (from compute_scatter_idx)
        remap_hidden_states: [num_tokens * topk, hidden_size] output buffer
        group: process group group_name: optional group name for symmetric
        memory backend_stream: optional stream for overlapping (multi-kernel
        fallback only) rank_buffers_ptr: optional precomputed buffer pointers
        (fused kernel only)
...
def allgather_with_symm_mem(
    ...
    if _HAS_ALLGATHER_WITH_SYMM_MEM_KERNEL:
        ...
        output_flat[
            rank * numel_per_rank : (rank + 1) * numel_per_rank
        ].copy_(my_slot)
        for step in range(world_size - 1):
            remote_rank = (rank - step - 1) % world_size
            remote_buffer = workspace.get_buffer(
            ...
            output_flat[
                remote_rank * numel_per_rank
                : (remote_rank + 1) * numel_per_rank
            ].copy_(remote_buffer)
